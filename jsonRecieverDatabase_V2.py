...
#deletes all sensor data from table
def deleteAllS(cursor):
    cursor.execute('DELETE FROM SensorData')
    
#deletes all setpoint data fro table
def deleteAllSP(cursor):
    cursor.execute('DELETE FROM iPotListing')
...
while True:
    ...
    #deleteAllS(cursorObj)
    #deleteAllSP(cursorObj)
    #conn.commit()
    #deleteAll(cursorObj)
    ...
    if pType == 1:
        if not wValue or len(time) == 0 or name is None:
            ...
            d.sendto(buf,('192.168.43.57',1007))
